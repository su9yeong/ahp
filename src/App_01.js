...
// rating bulletDescriptionHTML 등록
...
export default function App() {
  ...
  // 0=main → 1=hierarchy → 2=choice → 3=additional
  const model = useMemo(() => {
    const m = new Model(surveyJson);
    // 기본 버튼 숨기기
    m.showNavigationButtons = false;
    // 완료 화면(Thank you) HTML
    m.completedHtml = `
    ...
    `;
    // rating 후처리 & bulletDescriptionHTML
    ...
    m.onComplete.add((sender) => {
      ...
      fetch(WEBAPP_URL, {
        ...
        .then((r) => r.json())
        .then((j) => console.log("Sheets 저장 성공:", j))
        .catch((e) => console.error("Sheets 저장 실패:", e));
      // SurveyJS가 자동으로 completedHtml을 보여줍니다.
  ...
  // 필수 검증
  const validatePage = () => {
    const errors = model.currentPage.validate();
  ...
  const handlePrev = () => {
    ...
    if (stage === 4) {
      ...
      import(`./surveys/${chosenChoice}.js`).then((m) =>
  ...
  // → Survey가 completedHtml을 렌더링할 땐 isCompleted=true가 되어 nav 사라짐

  return (
    <div className="survey-wrapper">
      <Survey model={model} />

      {/* custom 내비게이션: 설문 완료 페이지만 제외 */}
      {showNav && (
        <div
          style={{
            display: "flex",
